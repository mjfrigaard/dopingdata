...
This vignette covers classification of single substances in the [USADA sanction data](https://www.usada.org/testing/results/sanctions/). 
...
1. **Scrape the website:** A copy of the [USADA sanctions table](https://www.usada.org/testing/results/sanctions/) has been scraped and lives in the `inst/extdata/raw/` folder 
  ...
  - See the `scrape_usada_sanctions()` function and `scraping-usada.Rmd` vignette for more information
    ...
    fs::dir_tree(
      path = paste0("../inst/extdata/raw/", tdy_stmp)
    )
    ```
...
``` r
get_recent_data_file("../inst/extdata/pro/", 
  type = "csv")
...
tidy_usada_sports <- readr::read_csv(
  file = '../inst/extdata/pro/2023-01-20/2023-01-20-usada_sanctions_pro.csv')
...
Export analytic sanctions:

```{r paths-usada_sanction_types, eval=FALSE}
new_pth <- paste0("../inst/extdata/", Sys.Date(), "/")
fs::dir_create(new_pth)
file_pth <- paste0(new_pth, dtstamp(side = "r"), 
                    "usada_sanction_types.csv")
file_pth
```

```{r xport-usada_sanction_types, eval=FALSE}
vroom::vroom_write(x = usada_sanction_types, file = file_pth, delim = ",")
```

```{r vrfy-usada_sanction_types, eval=FALSE}
fs::dir_tree(new_pth, regexp = "usada_sanction")
```


## Export substance data

```{r paths-usada_substance_types, eval=FALSE}
new_pth <- paste0("../inst/extdata/", tdy_stmp, "/")
file_pth <- paste0(new_pth, dtstamp(side = "r"), "usada_substance_types.csv")
file_pth
```

```{r xport-usada_substance_types, eval=FALSE}
vroom::vroom_write(x = usada_substance_types, file = file_pth, delim = ",")
```

```{r vrfy-usada_substance_types, eval=FALSE}
fs::dir_tree(new_pth, regexp = "usada_substance_types")
```
...
```{r export-classified_single_aaf_substance}
new_pth <- paste0("../inst/extdata/", tdy_stmp, "/")
file_pth <- paste0(new_pth, 
                  dtstamp(side = "r"), "classified_single_aaf_substance.csv")
file_pth
```

```{r xport-classified_single_aaf_substance, eval=FALSE}
vroom::vroom_write(x = classified_single_aaf_substance, 
  file = file_pth, delim = ",")
```

```{r vrfy-classified_single_aaf_substance}
fs::dir_tree(new_pth, regexp = "classified_single_aaf_substance")
```
