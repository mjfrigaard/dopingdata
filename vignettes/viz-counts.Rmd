...
`tidy_substances` is the live dataset built at the end of the ["Classifying substances"](classify.html)
article — fetched with `get_sanctions_data()`, then run through `process_text()`,
`clean_dates()`, `clean_sports()`, and `classify_wada_substances()`:
